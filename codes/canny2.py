...
def reference_detector():
    nbr_layer = 13 # write your number of layers here
    for i in range(2,13):
        image = cv2.imread('D:/academic/homework&assignment/senior design/data/edgedetection/data_3th/l%s/r.jpg' % i)
        edge_img, gradient = Canny(image, 0, 80)
        size = (256,255)
        img = cv2.resize(edge_img,size,interpolation = cv2.INTER_AREA)
        #cv2.imshow('', img)
        #cv2.waitKey(10000)
        cv2.imwrite('../../data/edgedetection/data_3th/l%s/re.jpg' % i, img)


def photos_detector():
    for i in range(301,500):
        image = cv2.imread('D:/academic/homework&assignment/senior design/data/testdata/img%s.jpg' % i)
        image = image[400:1200, 210:1150]
        #cv2.imshow('',image)
        #cv2.waitKey(3000)
        image, gradient = Canny(image, 0, 80)

        cv2.imshow('',image)
        cv2.waitKey(10000)
    #cv2.imwrite(os.path.join(output_path, filename), image)

        cv2.imwrite('D:/academic/homework&assignment/senior design/data/testdata2/img%s.jpg' % i, image)
